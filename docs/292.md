...
q<sup>*</sup>(s[、a<sub>)；【q】<sup>(s<sub>、a]</sup></sub></sub>

r[t+1]+[γ**max**<sub>a]Q[θ](s[t+1]，a)被认为是我们的目标，Q <sup>*</sup> (s[t] ，a[t] 是我们的网络预测的值。使用某种基于距离的损失函数(均方误差、Huber 损失等)。)，我们可以使用梯度下降来优化深度 Q 网络的权重。</sub>
