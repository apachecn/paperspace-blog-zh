...
> 原文:[https://blog.paperspace.com/train-yolov7-custom-data/](https://blog.paperspace.com/train-yolov7-custom-data/)
...
*   `source`标志定义了我们的检测器的来源，它通常是以下之一:单个图像、一个图像文件夹、一个视频、或者来自某个视频流或网络摄像头的实时反馈。我们想在我们的测试图像上运行它，所以我们将`source`标志设置为`Road_Sign_Dataset/images/test/`。
